    ...
    def forward(x, w, b, conv_param):
        ...
        for i in range(H_out):
          for j in range(W_out):
            ...
            for k in range(F):
              out[:, k, i, j] = torch.sum(x_pad_masked * w[k, :, :, :], dim = (1, 2, 3))
    ...
    def backward(dout, cache):
        ...
        for i in range(H_out):
          for j in range(W_out):
            x_pad_masked = x_pad[:, :, i*stride:i*stride+HH, j*stride:j*stride+WW]
            for k in range(F):
              dw[k, :, :, :] += torch.sum(x_pad_masked * (dout[:, k, i, j])[:, None, None, None], dim = 0)
            for n in range(N):
              dx_pad[n, :, i*stride:i*stride+HH, j*stride:j*stride+WW] += torch.sum((w[:, :, :, :] * (dout[n, :, i, j])[:, None, None, None]), dim = 0)
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        for i in range(H_out):
          for j in range(W_out):
            ...
            out[:, :, i, j] = torch.amax(x_masked, dim=(2, 3))
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
    ...
    def backward(dout, cache):
        ...
        for i in range(H_out):
          for j in range(W_out):
            x_masked = x[:, :, i*stride : i*stride+HH, j*stride : j*stride+WW]
            max_x_masked = torch.amax(x_masked, dim = (2, 3))
            mask_filter = (x_masked == (max_x_masked)[:, :, None, None])
            dx[:, :, i*stride:i*stride+HH, j*stride : j*stride+WW] += mask_filter * (dout[:, :, i, j])[:, :, None, None]
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        self.params['W2'] = weight_scale * torch.randn((H // 2)*(W // 2)*num_filters, hidden_dim, dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        conv_forward_out_1, cache_forward_1 = Conv_ReLU_Pool.forward(X, self.params['W1'], self.params['b1'], conv_param, pool_param)
        affine_forward_out_2, cache_forward_2 = Linear_ReLU.forward(conv_forward_out_1, self.params['W2'], self.params['b2'])
        scores, cache_forward_3 = Linear.forward(affine_forward_out_2, self.params['W3'], self.params['b3'])
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        dX3, grads['W3'], grads['b3'] = Linear.backward(dout, cache_forward_3)
        dX2, grads['W2'], grads['b2'] = Linear_ReLU.backward(dX3, cache_forward_2)
        dX1, grads['W1'], grads['b1'] = Conv_ReLU_Pool.backward(dX2, cache_forward_1)

        grads['W3'] = grads['W3'] + 2 * self.reg * self.params['W3']
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        prev_filters, H_out, W_out = input_dims
        ...
        for i, num_filter in enumerate(num_filters):
          ...
          if weight_scale == 'kaiming':
            self.params['W%d'%i] = kaiming_initializer(num_filter, prev_filters, K=filter_size, relu=True, device=device,dtype=dtype)
          else:
            self.params['W%d'%i] = weight_scale * torch.randn(num_filter, prev_filters, filter_size, filter_size, dtype = dtype, device = device)
            
          self.params['b%d'%i] = torch.zeros(num_filter, dtype=dtype, device = device)
          prev_filters = num_filter

        i += 1
        if weight_scale == 'kaiming':
          self.params['W%d'%i] = kaiming_initializer(num_filter*H_out*W_out, num_classes,relu=False, device=device,dtype=dtype)
        else:
          self.params['W%d'%i] = weight_scale * torch.randn(num_filter*H_out*W_out, num_classes, dtype = dtype, device = device)
        
        self.params['b%d'%i] = torch.zeros(num_classes, dtype=dtype, device=device)
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            gammax = gamma * xhat
            out = gammax + beta
            cache = (xhat, gamma, xmu, ivar, sqrt_var, var, eps)
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
        elif mode == 'test':
