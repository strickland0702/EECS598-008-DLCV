...
def rnn_step_backward(dnext_h, cache):
    ...
    # z is the input of tanh function
    dz = dnext_h * (1 - next_h.pow(2))
    dx = dz.mm(Wx.t())
    dprev_h = dz.mm(Wh.t())
    dWx = (x.t()).mm(dz)
    dWh = (prev_h.t()).mm(dz)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    prev_h = h0
    for i in range(T):
        next_h, local_cache = rnn_step_forward(x[:, i], prev_h, Wx, Wh, b)
...
def rnn_backward(dh, cache):
    ...
    for i in range(T-1, -1, -1):
        dh_sum = dh[:, i] + dprev_h
        local_dx, dprev_h, local_dWx, local_dWh, local_db = rnn_step_backward(dh_sum, cache[i])
        dx[:, i] = local_dx
        dWh += local_dWh
        dWx += local_dWx
        db += local_db
