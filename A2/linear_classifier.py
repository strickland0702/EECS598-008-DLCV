...
def svm_loss_vectorized(
    ...
    num_train = X.shape[0]
    scores = X.mm(W)
    correct_class_scores = scores[torch.arange(num_train), y].reshape(num_train, 1)
    ...
    # Replace "pass" statement with your code
    margin[margin > 0] = 1
    margin[torch.arange(num_train), y] -= margin.sum(axis = 1)
...
def sample_batch(
    ...
    #########################################################################
    # Replace "pass" statement with your code
    indexes = torch.randint(0, num_train, (batch_size, ))
    X_batch = X[indexes, :]
    y_batch = y[indexes]
    #########################################################################
    #                       END OF YOUR CODE                                #
...
def svm_get_search_params():
    ...
    regularization_strengths = [1e-2, 5e-2, 0.1, 0.5, 1]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def test_one_param_set(
    ...
    train_acc = torch.sum(y_train_pred == y_train) / num_train 
    y_val_pred = cls.predict(X_val)
    val_acc = torch.sum(y_val_pred == y_val) / num_val
...
def softmax_loss_naive(
    ...
    scores = X.mm(W)
    for i in range(num_train):
        score = scores[i] - torch.max(scores[i])
        softmax_score = torch.exp(score) / torch.exp(score).sum()
        loss += -torch.log(softmax_score[y[i]])
        for j in range(num_classes):
...
def softmax_loss_vectorized(
    ...
    softmax_scores = torch.exp(scores)
    softmax_scores = softmax_scores / softmax_scores.sum(dim = 1, keepdim=True)
    loss = -torch.log(softmax_scores[torch.arange(num_train), y]).sum()

    softmax_scores[torch.arange(num_train), y] -= 1
    dW = X.t().mm(softmax_scores)
